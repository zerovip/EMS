    ...
    def read_out(self, choose, start, end=time.strftime("%Y-%m-%d %H:%M", time.localtime())):
        ...
        for item in choose:
            ...
            for time_it in a:
                ...
                time = time_it[11:]
                try:
                    rt_data[item].append(Data_da.temp['{0}_{1}_{2}'.format(item[0], Device.objects.get(id=item[0]).name, date)][time][int(item[1])])
                except:
                    try:
                        read_f = open("./data_db/{0}_{1}_{2}.zero".format(item[0], Device.objects.get(id=item[0]).name, date), "r")
